...
//맨 아래 기재된 출력값을 참고하여 코드를 작성하는 문제이다.
import java.util.*;

public class KimEx06_Answer {
    ...
    static int score; //[문제1] score 변수를 오류 나지 않도록 작성하세요.
    WordGenerator wg = new WordGenerator();

 public static void main(String args[]) {
        KimEx06_Answer game = new KimEx06_Answer();
        game.wg.start();
        ...
        while (true) {
            ...
            if(input.equals("그만")) {
                game.wg.interrupt();//인터럽트 걸었음 //[문제2] 이전의 상태로 복귀시키는 코드를 작성하세요. (힌트:인터럽트)
...
public class WordGenerator extends Thread {
        public void run() {
            while (true) {
                ...
                words.add(data[rand]); //[문제3]배열 data의 값 중 하나를 임의로 선택해서 word에 추가하는 코드를 작성하세요.

                try {
                    Thread.sleep(interval); //[문제4]interval을 일시중단하는 코드를 작성하세요.
                } catch (Exception e) {
